...
export function Footer() {
  return (
    <motion.footer
      ...
      style={{
        borderImage: "linear-gradient(to right, #F5A623, #FF5C35, #C8F135) 1",
      }}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        ...
        <div>
          {config.useImageLogo ? (
            ...
            <h2 className="font-display text-6xl text-primary-gold tracking-widest mb-2">
              PEX RUM
            </h2>
          )}
          <p className="font-accent italic text-cream text-lg mb-4">
        ...
        <div>
          ...
          <ul className="space-y-4">
            {["Home", "About", "Contact", "Stores"].map((link) => (
              <li key={link}>
                <Link
                  to={
                    link === "Home"
      ...
      <div className="border-t border-[#1A0C00] py-6 px-6 md:px-12 flex flex-col md:flex-row justify-between items-center gap-4">
        <p className="font-label text-xs text-text-muted">
          © {new Date().getFullYear()} Pex Rum. All rights reserved. Drink
          Responsibly.
        </p>
        <p className="font-label text-xs text-text-muted">
